...
| Convert field to epoch for sorting, but display as human readable                                        | `\| eval Date=strptime(Date, "%m/%d/%Y %I:%M:%S %p") \| fieldformat Date=strftime(Date, "%m/%d/%Y %I:%M:%S %p")`             |
| Change time field display to show ISO 8601 with Z (Deliberately want a string, e.g. for export)       | `\| eval _time = strftime(_time, "%Y-%m-%d %H:%M:%S")`                                                                       |
...
| Change time field display to show ISO 8601 with Z (Display only, or display + keep using numerically) | `\| fieldformat _time = strftime(_time, "%Y-%m-%dT%H:%M:%SZ")`                                                               |
...
| Convert epoch date (numbers) to date                                                                  | `\| convert ctime(DateField)`                                                                                                |
